...
while not done:
    ...
    if not locked:                              
        if keys[pygame.K_s]:                                # Controls for the player Ashe which uses the keys w,a,s,d
            ...
            if (ya+1) < bottomWall:                         # This is to check the space available to make sure Ashe doesn't walk into the walls
                ya += 1
    ...
    if not locked:
        if keys[pygame.K_DOWN]:                             # Controls for the player Misty which uses arrow keys on the right side of the keyboard
            ...
            if (y+1) < bottomWall:                          # This is to check for the space available to make sure Misty doesn't walk into the walls     
                y += 1
